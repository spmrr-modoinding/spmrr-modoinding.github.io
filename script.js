...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ========================================================
  // === BAGIAN 2: FUNGSI BARU UNTUK MEMUAT AGENDA DARI FIREBASE ===
  // ========================================================
  const loadAnnouncementsPublic = async () => {
      ...
      try {
                  ...
                  <div class="col-lg-6 col-md-12 mb-4">
                    <div class="card shadow-sm h-100 agenda-card">
                      <div class="card-body d-flex flex-column">
                        ...
                        <a href="#" class="btn btn-sm btn-outline-primary agenda-detail-btn mt-auto" data-glightbox="title: ${item.judul.replace(/"/g, '&quot;')}; description: ${glightboxContent.replace(/"/g, '&quot;')}">Lihat Detail <i class="bi bi-arrow-right-circle-fill ms-1"></i></a>
                      </div>
                    </div>
                  </div>`;
          }).join('');
          ...
          container.innerHTML = `<p class="text-danger">Terjadi kesalahan saat memuat agenda. Pastikan Aturan Keamanan (Security Rules) di Firebase sudah benar. Error: ${error.message}</p>`;
      }
  };

  // --- Memanggil fungsi baru untuk agenda ---
  loadAnnouncementsPublic();
  
  // --- Kode Anda yang lainnya tetap dipertahankan ---
  ...
  // Liturgi Mingguan & Renungan (tetap dari JSON)
  fetch('liturgi_mingguan.json').then(res => res.json()).then(data => {
      ...
      if (bacaanContainer && renunganContainer) {
          ...
          bacaanContainer.innerHTML = `<div class="liturgi-card"><div class="liturgi-header"><span class="liturgi-date">${minggu_ini.tanggal}</span><span class="liturgi-color-dot ${colorMap[minggu_ini.warna] || 'default'}"></span></div><div class="liturgi-body"><h5 class="liturgi-perayaan">${minggu_ini.peringatan}</h5><div class="liturgi-detail"><p><i class="bi bi-book-fill me-2"></i><strong>Bacaan 1:</strong> ${minggu_ini.bacaan1}</p><p><i class="bi bi-book-fill me-2"></i><strong>Bacaan 2:</strong> ${minggu_ini.bacaan2}</p><p><i class="bi bi-music-note-beamed me-2"></i><strong>Mazmur:</strong> ${minggu_ini.mazmur}</p><p><i class="bi bi-journal-medical me-2"></i><strong>Injil:</strong> ${minggu_ini.injil}</p><p><i class="bi bi-palette-fill me-2"></i><strong>Warna:</strong> ${minggu_ini.warna}</p></div></div></div>`;
  ...
  // BAGIAN UNTUK AGENDA DARI JSON SUDAH DIHAPUS DAN DIGANTIKAN DENGAN FUNGSI BARU DI ATAS
  ...
  fetch('statistik_umat.json').then(res => res.json()).then(data => {
      ...
      if (container) {
          ...
          container.innerHTML = `<h2>Statistik Umat per Tahun</h2><div class="overflow-auto"><table class="stats-table"><thead><tr><th>No</th><th>Nama Wilayah</th><th>KK</th><th>Laki-laki</th><th>Perempuan</th><th>Jumlah Jiwa</th></tr></thead><tbody>${rows}${totalRow}</tbody></table></div>`;
      }
  }).catch(console.error);
  ...
  fetch('pastor_kehadiran.json').then(res => res.json()).then(data => {
      ...
      if (container) {
          const statusItems = data.pastor.map(p => {
              ...
              return `<div><img alt="${p.nama}" class="pastor-photo" src="${p.foto}" /><p>${p.nama}</p><span class="indicator ${hadir ? 'green' : 'red'}">${hadir ? 'Di Tempat' : 'Tidak di Tempat'}</span></div>`;
          }).join('');
          container.innerHTML = `<h2>Kehadiran Pastor Hari Ini</h2><div class="pastor-status">${statusItems}</div><p class="mt-3" style="font-size: 0.9rem; color: #555;">Keterangan:<br/><span class="indicator green" style="padding: 2px 10px;">Di Tempat</span> = Pastor berada di pastoran.<br/><span class="indicator red" style="padding: 2px 10px;">Tidak di Tempat</span> = Pastor sedang pelayanan di luar.</p>`;
      }
  }).catch(console.error);
